...
## 2026-04-16 - [Privilege Escalation in Quality Profiles]
**Vulnerability:** Non-admin users could set a quality profile as the system default during creation or update, which unsets the current global default.
**Learning:** Role-based access control (RBAC) was missing for sensitive fields (like `IsDefault`) in otherwise user-accessible handlers. Authentication alone is insufficient for fields that affect global system state.
**Prevention:** Explicitly check user roles in handlers when processing requests that modify system-wide settings or administrative flags, and return 403 Forbidden for unauthorized attempts.
