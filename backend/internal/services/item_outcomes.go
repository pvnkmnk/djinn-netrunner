...
//
// All three return the write error. A verdict that was never recorded must not
// read as handled: an item whose status write fails stays claimable, which is
// recoverable, while a caller that swallowed the error would report success
// and leave the accounting lying.
...
// It returns the write error, so a caller cannot treat an outcome that was
// never recorded as handled — an item whose verdict write failed stays
// claimable, which is recoverable, rather than silently final.
func (h *AcquisitionHandler) noResultsItem(jobID uint64, itemID uint64, reason string) error {
	h.Log(jobID, "ERR", reason, &itemID)
	return h.db.Model(&database.JobItem{}).Where("id = ?", itemID).Updates(map[string]interface{}{
...
func (h *AcquisitionHandler) failItem(jobID uint64, itemID uint64, reason string) error {
	h.Log(jobID, "ERR", reason, &itemID)

	var item database.JobItem
	if err := h.db.First(&item, itemID).Error; err != nil {
		return fmt.Errorf("load item %d to fail it: %w", itemID, err)
	}

	// Check job-level max attempts to determine if item should be abandoned.
	// A job that cannot be loaded is an error, not permission to schedule
	// another retry: MaxAttempts is what stops a failing item from retrying
	// forever, so guessing past a failed lookup could defeat the limit.
	var job database.Job
	if err := h.db.First(&job, jobID).Error; err != nil {
		return fmt.Errorf("load job %d to check the attempt limit: %w", jobID, err)
	}
