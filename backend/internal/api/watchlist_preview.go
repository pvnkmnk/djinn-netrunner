...
func (h *WatchlistPreviewHandler) GetPreview(c *fiber.Ctx) error {
	// Auth check - consistent with other handlers
	...
	// BOLA protection: user must own the watchlist or be admin
	if user.Role != "admin" && (watchlist.OwnerUserID == nil || *watchlist.OwnerUserID != user.ID) {
		return c.Status(fiber.StatusForbidden).SendString("forbidden: you do not own this watchlist")
	}
