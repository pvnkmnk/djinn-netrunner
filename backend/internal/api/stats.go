...
func (h *StatsHandler) GetLibraryStats(c *fiber.Ctx) error {
	...
	if user.Role != "admin" {
		formatQuery = formatQuery.Joins("JOIN libraries ON libraries.id = tracks.library_id").Where("libraries.owner_user_id = ?", user.ID)
	}
	...
	// Library breakdown
	libBreakdownQuery := h.db.Model(&database.Track{})
	if user.Role != "admin" {
		libBreakdownQuery = libBreakdownQuery.Joins("JOIN libraries ON libraries.id = tracks.library_id").Where("libraries.owner_user_id = ?", user.ID)
	}
	libBreakdownQuery.Select("library_id, COUNT(*) as track_count, COALESCE(SUM(file_size), 0) as total_size").
		Group("library_id").
	...
	libListQuery := h.db
	if user.Role != "admin" {
		libListQuery = libListQuery.Where("owner_user_id = ?", user.ID)
	}
	libListQuery.Find(&libraries)
	...
	if user.Role == "admin" {
		...
		err = h.db.Raw(`
			SELECT
				...
				(SELECT COUNT(*) FROM quality_profiles WHERE owner_user_id = ? OR is_default = 1) as quality_profiles,
	...
	if user.Role == "admin" {
		...
		h.db.Raw(`
			SELECT
				...
				(SELECT COUNT(*) FROM libraries WHERE owner_user_id = ?) as libraries,
				(SELECT COUNT(*) FROM jobs WHERE requested_at > ? AND owner_user_id = ?) as recent_jobs24h
		`, user.ID, user.ID, user.ID, user.ID, since24h, user.ID).Scan(&summary.Activity)
	}
