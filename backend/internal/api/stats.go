...
func (h *StatsHandler) GetLibraryStats(c *fiber.Ctx) error {
	...
	totalQuery := h.db.Model(&database.Track{})
	if user.Role != "admin" {
		totalQuery = totalQuery.Joins("JOIN libraries ON libraries.id = tracks.library_id").
			Where("libraries.owner_user_id = ?", user.ID)
	}

	totalQuery.Select("COUNT(*) as total_tracks, COALESCE(SUM(file_size), 0) as total_size").
		Scan(&totals)
	...
	// Library breakdown
	libraryBreakdownQuery := h.db.Model(&database.Track{})
	if user.Role != "admin" {
		libraryBreakdownQuery = libraryBreakdownQuery.Joins("JOIN libraries ON libraries.id = tracks.library_id").
			Where("libraries.owner_user_id = ?", user.ID)
	}

	libraryBreakdownQuery.Select("tracks.library_id, COUNT(*) as track_count, COALESCE(SUM(file_size), 0) as total_size").
		Group("tracks.library_id").
		Scan(&stats.LibraryBreakdown)

	// Get library names in a single query to avoid N+1
	var libraries []database.Library
	libraryQuery := h.db.Model(&database.Library{})
	if user.Role != "admin" {
		libraryQuery = libraryQuery.Where("owner_user_id = ?", user.ID)
	}
	libraryQuery.Find(&libraries)
	...
	if user.Role == "admin" {
		...
		err = h.db.Raw(`
			SELECT
				...
				(SELECT COUNT(*) FROM quality_profiles WHERE owner_user_id = ? OR is_default = true) as quality_profiles,
		...
		`, user.ID, user.ID, user.ID, user.ID, since24h, user.ID, since7d, user.ID).Scan(&stats).Error
	}
	...
	if user.Role == "admin" {
		...
		err = h.db.Raw(`
			SELECT
				...
				(SELECT COUNT(*) FROM libraries WHERE owner_user_id = ?) as libraries,
				(SELECT COUNT(*) FROM jobs WHERE requested_at > ? AND owner_user_id = ?) as recent_jobs24h
		`, user.ID, user.ID, user.ID, user.ID, since24h, user.ID).Scan(&summary.Activity).Error
	}
