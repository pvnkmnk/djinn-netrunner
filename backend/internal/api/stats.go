...
func (h *StatsHandler) GetActivityStats(c *fiber.Ctx) error {
	...
	// Bolt Optimization: Consolidate multiple count queries into a single SQL query
	// using subqueries to reduce database roundtrips from 6 to 1.
	err := h.db.Raw(`
		SELECT
...
func (h *StatsHandler) GetSummary(c *fiber.Ctx) error {
	...
	// Activity stats
	since24hActivity := time.Now().Add(-24 * time.Hour)

	// Bolt Optimization: Consolidate multiple count queries into a single SQL query
	// using subqueries to reduce database roundtrips from 5 to 1.
	h.db.Raw(`
		SELECT
			(SELECT COUNT(*) FROM monitored_artists) as monitored_artists,
			(SELECT COUNT(*) FROM watchlists) as watchlists,
			...
			(SELECT COUNT(*) FROM libraries) as libraries,
			(SELECT COUNT(*) FROM jobs WHERE requested_at > ?) as recent_jobs24h
	`, since24hActivity).Scan(&summary.Activity)
