...
func setupRoutes(app *fiber.App, db *gorm.DB, auth *api.AuthHandler, dash *api.DashboardHandler, stats *api.StatsHandler, library *api.LibraryHandler, profile *api.ProfileHandler, watchlist *api.WatchlistHandler, watchlistService *services.WatchlistService, spotifyAuth *api.SpotifyAuthHandler, ws *api.WebSocketManager, at *services.ArtistTrackingService, scan *services.ScannerService, artistsHandler *api.ArtistsHandler, schedulesHandler *api.SchedulesHandler) {
	...
	// Page routes
	app.Get("/watchlists", watchlist.WatchlistsPage)
	app.Get("/libraries", library.LibrariesPage)
	app.Get("/profiles", profile.ProfilesPage)
	app.Get("/schedules", schedulesHandler.SchedulesPage)
	app.Get("/artists", artistsHandler.ArtistsPage)
	app.Get("/jobs", stats.JobsPage)

	// Partial routes (all protected - auth handled inside each handler)
	app.Get("/partials/stats", stats.RenderStatsPartial)
	app.Get("/partials/watchlists", watchlist.RenderWatchlistsPartial)
	app.Get("/partials/libraries", library.RenderLibrariesPartial)
	app.Get("/partials/schedules", schedulesHandler.RenderSchedulesPartial)
	app.Get("/partials/artists", artistsHandler.RenderPartial)
