...
// Uses the MusicBrainz API: GET /ws/2/artist/{mbid}?inc=release-groups
func (s *MusicBrainzService) GetArtistDiscography(artistID string) (map[string]interface{}, error) {
	...
	// Build the correct endpoint: artist/{mbid} with inc=release-groups
	endpoint := fmt.Sprintf("artist/%s", artistID)
	params := url.Values{}
	params.Add("inc", "release-groups")
	params.Add("fmt", "json")

	result, err := s.doRequest(endpoint, params)
	if err != nil {
		return nil, err
	}
